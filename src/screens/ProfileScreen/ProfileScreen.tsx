...
const ProfileScreen = () => {
  ...
  const calculateProfilePercentage = () => {
    ...
    const totalFields = 5; // name, gender, age, height, weight
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: ourColors.background }]}>
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
          <Text style={{ color: ourColors.text }}>Yükleniyor...</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: ourColors.background }]}>
      ...
      <View style={[styles.header, { 
        ...
        >
          <SvgXml 
            xml={settingsIcon.replace(/#000000/g, ourColors.text)} 
            width={24} 
            height={24} 
          />
        </TouchableOpacity>
      ...
      >
        {/* Profile Section */}
        <View style={[styles.profileSection, { backgroundColor: ourColors.background }]}>
          <View style={[styles.profileImageContainer, {
            ...
            <Svg width={116} height={116} style={[styles.progressCircle, {
              ...
              <Circle
                ...
                cy="58"
                r="56"
                stroke={ourColors.primary}
                strokeWidth="3"
                fill="none"
                strokeDasharray={2 * Math.PI * 56}
                strokeDashoffset={2 * Math.PI * 56 * (1 - calculateProfilePercentage() / 100)}
                strokeLinecap="round"
              />
            </Svg>
            <View style={[styles.progressRing, { 
              ...
              <Text style={[styles.progressText, { 
                ...
                textAlign: 'center'
              }]}>{calculateProfilePercentage()}%</Text>
            </View>
          </View>
          
          <View style={styles.profileInfo}>
            <Text style={[styles.name, { color: ourColors.text }]}>
              {userData?.displayName || t('profile.default_user_name')}
            </Text>
          </View>
          ...
          <TouchableOpacity 
            ...
            onPress={() => (navigation as any).navigate('BioScreen')}
          >
            <Text style={[styles.editBioText, { 
              ...
              fontWeight: '600',
              marginRight: 8
            }]}>{t('profile.edit_bio')}</Text>
            <Text style={{ color: '#FFFFFF', fontSize: 18 }}>✎</Text>
          </TouchableOpacity>
          
          <Text style={[styles.subtitle, { color: ourColors.textSecondary }]}>{t('profile.personal_info')}</Text>
        </View>

        {/* Debug: Async Storage Verileri */}
        <View style={[styles.section, { backgroundColor: ourColors.background }]}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: ourColors.text }]}>🔍 Async Storage Debug</Text>
          </View>
          ...
          <View style={[styles.debugContainer, { 
            ...
            <Text style={[styles.debugTitle, { color: ourColors.text, fontSize: 14, fontWeight: '600', marginBottom: 8 }]}>
              User Data (JSON):
            </Text>
            <ScrollView style={{ maxHeight: 200 }} showsVerticalScrollIndicator={false}>
              <Text style={[styles.debugText, { 
                ...
                {userData ? JSON.stringify(userData, null, 2) : 'Veri yok'}
              </Text>
            </ScrollView>
          </View>
        </View>
        ...
        <View style={[styles.section, { backgroundColor: ourColors.background }]}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: ourColors.text }]}>{t('profile.personal_data')}</Text>
          </View>
          
          <TouchableOpacity 
            style={[styles.menuItem, { borderBottomColor: ourColors.cardBorder }]}
            onPress={() => console.log('Age tapped')}
          >
            <Text style={[styles.menuItemText, { color: ourColors.text }]}>{t('profile.age')}</Text>
            <View style={styles.menuItemRight}>
              {userData?.age && userData.age > 0 ? (
                <>
                  <Text style={[styles.menuItemSubtext, { color: ourColors.primary }]}>{userData.age} yaşında</Text>
                  <Text style={[styles.menuItemAction, { color: ourColors.textSecondary }]}>Değiştir</Text>
                </>
              ) : (
                <Text style={[styles.menuItemAction, { color: ourColors.textSecondary, opacity: 0.7 }]}>Ekle</Text>
              )}
          ...
          <TouchableOpacity 
            style={[styles.menuItem, { borderBottomColor: ourColors.cardBorder }]}
            onPress={() => console.log('Gender tapped')}
          >
            <Text style={[styles.menuItemText, { color: ourColors.text }]}>{t('profile.gender')}</Text>
            <View style={styles.menuItemRight}>
              {userData?.gender !== null ? (
                <>
                  <Text style={[styles.menuItemSubtext, { color: ourColors.primary }]}>
                    {userData?.gender === 0 ? 'Erkek' : 'Kadın'}
                  </Text>
                  <Text style={[styles.menuItemAction, { color: ourColors.textSecondary }]}>Değiştir</Text>
                </>
              ) : (
          ...
          <TouchableOpacity 
            style={[styles.menuItem, { borderBottomColor: ourColors.cardBorder }]}
            onPress={() => console.log('Weight tapped')}
          >
            <Text style={[styles.menuItemText, { color: ourColors.text }]}>{t('profile.weight')}</Text>
            <View style={styles.menuItemRight}>
              {userData?.weight && userData.weight?.kg > 0 ? (
                <>
                  <Text style={[styles.menuItemSubtext, { color: ourColors.primary }]}>{userData.weight?.kg} kg</Text>
                  <Text style={[styles.menuItemAction, { color: ourColors.textSecondary }]}>Değiştir</Text>
                </>
              ) : (
                <Text style={[styles.menuItemAction, { color: ourColors.textSecondary, opacity: 0.7 }]}>Ekle</Text>
          ...
          <TouchableOpacity 
            style={[styles.menuItem, { borderBottomColor: ourColors.cardBorder }]}
            onPress={() => console.log('Height tapped')}
          >
            <Text style={[styles.menuItemText, { color: ourColors.text }]}>{t('profile.height')}</Text>
            <View style={styles.menuItemRight}>
              {userData?.height && userData.height?.cm > 0 ? (
                <>
                  <Text style={[styles.menuItemSubtext, { color: ourColors.primary }]}>{userData.height?.cm} cm</Text>
                  <Text style={[styles.menuItemAction, { color: ourColors.textSecondary }]}>Değiştir</Text>
                </>
              ) : (
                <Text style={[styles.menuItemAction, { color: ourColors.textSecondary, opacity: 0.7 }]}>Ekle</Text>
...
const styles = StyleSheet.create({
  ...
  debugContainer: {
    // Style tanımları yukarıda inline olarak verildi
  },
  debugTitle: {
    // Style tanımları yukarıda inline olarak verildi
  },
